...
export default function PrintBatchQC({ reports }: { reports: QCEntry[] }) {
  return (
    <div style={{ background: '#fff', color: '#000', minHeight: '100vh', padding: '16px' }}>
      ...
      <div className="no-print" style={{ marginBottom: '20px', display: 'flex', gap: '8px', justifyContent: 'center', padding: '12px', background: '#f9fafb', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
        <span style={{ fontSize: '14px', color: '#374151', alignSelf: 'center' }}>
          {reports.length} QC Report{reports.length !== 1 ? 's' : ''}
        </span>
        <button
          onClick={() => window.print()}
          style={{ padding: '8px 20px', background: '#0ea5e9', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: 600 }}
        >
          Print All
        </button>
        <button
      ...
      {reports.map((r, idx) => {
        const entries = Object.entries(r.qcData);
        ...
        const dateStr = new Date(r.updatedAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' });
        const timeStr = new Date(r.updatedAt).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' });

        return (
          <div key={idx} className={idx > 0 ? 'page-break' : ''} style={{ maxWidth: '700px', margin: '0 auto', fontFamily: 'Arial, sans-serif', marginBottom: idx < reports.length - 1 ? '40px' : '0' }}>
            {/* Header */}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', fontSize: '12px', marginBottom: '20px' }}>
              <div>
                ...
                <div style={{ fontWeight: 600, fontFamily: 'monospace', fontSize: '13px' }}>{r.barcode}</div>
              </div>
              <div>
              ...
              <div>
                ...
                <div style={{ fontWeight: 600 }}>{r.assignedTo}</div>
              </div>
              <div>
