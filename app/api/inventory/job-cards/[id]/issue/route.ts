...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!['INVENTORY_MANAGER', 'STORE_MANAGER', 'PRODUCTION_EMPLOYEE', 'PRODUCTION_MANAGER', 'ADMIN'].includes(session.role)) {
  ...
  // Mark job card as issued
  const updated = await prisma.jobCard.update({
    where: { id },
    data: { status: 'ISSUED', issuedById: session.id, issuedAt: new Date() },
    include: {
