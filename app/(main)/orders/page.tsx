...
export default async function OrdersPage() {
  ...
  const [rawOrders, products, clients, myJobCards] = await Promise.all([
    prisma.order.findMany({
      include: {
        ...
        units: { select: { currentStatus: true, currentStage: true, product: { select: { productType: true, name: true } } } },
      },
  ...
  const orders: OrderItem[] = rawOrders.map((o) => ({
    ...
    units: o.units.map((u) => ({ currentStatus: u.currentStatus, currentStage: u.currentStage, isTrading: u.product?.productType === 'TRADING', productName: u.product?.name })),
